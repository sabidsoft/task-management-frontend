...
import { useUpdateTaskMutation } from "../../redux/features/api/taskApi/taskApi";
...
export default function EditTask() {
    ...
    const [errorMessage, setErrorMessage] = useState(initialCompleted);
    const [updateTour, { isSuccess, error, isLoading }] = useUpdateTaskMutation();

    const onSubmit: SubmitHandler<TaskFormData> = (data) => {
        updateTour({ taskId, formData: data });
    };
    ...
    return (
        <div className="bg-[#2DCAC2] min-h-screen flex justify-center items-center">
            <div className="w-[350px] bg-[#fff] rounded-xl mx-auto py-4 px-5">
                <h1 className="uppercase text-center font-semibold text-3xl text-[#2DCAC2] mb-5">Edit Task</h1>
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="flex justify-center">
                        {isLoading ? (
                            ...
                            <input type="submit" value="Edit Task" className="bg-[#2DCAC2] inline-block text-white font-medium mt-5 mb-2 py-2 px-4 rounded" />
                        )}
                    </div>
                </form>
            </div>
